...
- 修复 Pydantic V1 环境下查询必定报错的问题（模型无法按字段名构造），配置键改用 V2 全系通用写法
- 修复 `02-29` 无法解析的问题，并改用不依赖「无年份日期解析」的写法（该行为自 Python 3.14 起弃用、3.15 将变更）
- `CNRAIL_ACG_IMAGE_URL` 现在会校验是否为合法的 http(s) 链接，配置非法时插件加载即报错
- 车组号查询失败不再导致整次查询失败，仅不显示车组号
- 修复 `-h` 与参数错误的回复未带上「使用指令 `train -h` 查看帮助」提示的问题
- 新增 pytest 测试套件与 CI

### 0.4.2

- 修复 Pydantic V2 下查询必定报错的问题（模型无法按字段名构造）
